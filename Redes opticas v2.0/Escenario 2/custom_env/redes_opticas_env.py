...
class RedesOpticasEnv(gym.Env):
    ...
    def __init__(self, render_mode=None,seed=0, num_ont=3, v_max_olt=10e6, vt_contratada=10e6/10, lista_resultado=None, n_ciclos=200):
    ...
    def calculate_pareto(self, num_ont=5, traf_pas=[]):
        ...
        for i in range(num_ont):
            if not traf_pas:
                ...
                trafico_pareto = list(self.rng.pareto(alpha_on_value, size=(1)))
            ...
            while suma < 2:
                alpha_on_value = alpha_on[i % len(alpha_on)]
                trafico_pareto += list(self.rng.pareto(alpha_on_value, size=(1))) + list(self.rng.pareto(alpha_off, size=(1)))
                suma = sum(trafico_pareto)
